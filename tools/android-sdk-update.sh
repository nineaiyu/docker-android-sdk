...
then
    ...
    mkdir -p cmdline-tools/latest/ \
      && curl -sSL https://dl.google.com/android/repository/commandlinetools-linux-7583922_latest.zip -o sdk-tools-linux.zip \
...
then
    ...
    echo "install clubhouse"
    done < /opt/tools/package-list-clubhouse.txt
else
    while read p; do
      android-accept-licenses.sh "sdkmanager ${SDKMNGR_OPTS} ${p}"
